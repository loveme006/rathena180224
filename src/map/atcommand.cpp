...
ACMD_FUNC(mobinfo)
{
	...
	for (k = 0; k < count; k++) {
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			if (event_dflag == 1) droprate += (droprate / 10) * event_rate; // [Nerguri] 이벤트 명령어
#ifdef RENEWAL_DROP
			if( battle_config.atcommand_mobinfo_type ) {
		...
		if (mob->mexp) {
			...
			for (i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
				...
				if (mvppercent > 0) {
					if (event_dflag == 1) mvppercent += (mvppercent / 10) * event_rate; // [Nerguri] 이벤트 명령어
					j++;
					if (j == 1) {
						if (item_data->slot)
...
#include "../custom/atcommand_def.inc"
		...
		ACMD_DEF2("aloottype", autoloottype),
		ACMD_DEF(mobinfo),
		ACMD_DEF(exp),
		ACMD_DEF(version),
		ACMD_DEF(mutearea),
