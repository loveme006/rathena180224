...
//	s_random_opt_group_entry *eTotal = NULL;
//	if (!itm || !mobdrop || mobdrop->randomopt_group[0] == RDMOPTG_None)
//		return;
//	if ((g = itemdb_randomopt_group_exists(mobdrop->randomopt_group[0])) && g->total) {
//		int r = rnd() % g->total;
//		if (&g->entries[r]) {
//			memcpy(&itm->option, &eTotal, sizeof(itm->option));
//			return;
//			ShowWarning("0: %d\n", mobdrop->randomopt_group[0]);
//			if(mobdrop->randomopt_group[1] != 0)
//				if ((g = itemdb_randomopt_group_exists(mobdrop->randomopt_group[1])) && g->total) {
//					ShowWarning("1: %d\n", mobdrop->randomopt_group[1]);
//					int r = rnd() % g->total;
//					if (&g->entries[r]) {
//						memcpy(&itm->option + sizeof(itm->option), &eTotal, sizeof(itm->option));
//					}
//				}
//			if (mobdrop->randomopt_group[2] != 0)
//				if ((g = itemdb_randomopt_group_exists(mobdrop->randomopt_group[2])) && g->total) {
//					ShowWarning("2: %d\n", mobdrop->randomopt_group[2]);
//					int r = rnd() % g->total;
//					if (&g->entries[r]) {
//						memcpy(&itm->option + sizeof(itm->option)*2, &eTotal, sizeof(itm->option));
//					}
//				}
//		}
//	}
...
void mob_setdropitem_option(struct item *itm, struct s_mob_drop *mobdrop) {
	...
	if ((g = itemdb_randomopt_group_exists(mobdrop->randomopt_group[0])) && g->total) {
		...
		int tmp1 = 0;
		if (&g->entries[r]) {
			g->entries[r].option[0].value = rnd_value(g->entries[r].option[0].value, (short)g->entries[r].option[0].param);
			...
			if (mobdrop->randomopt_group[1] != 0)
				if ((g = itemdb_randomopt_group_exists(mobdrop->randomopt_group[1])) && g->total) {
					...
					if (&g->entries[r]) {
						g->entries[r].option[0].value = rnd_value(g->entries[r].option[0].value, (short)g->entries[r].option[0].param);
						memcpy(&itm->option[1], &g->entries[r].option[0], sizeof(s_item_randomoption));
					}
				}
			if (mobdrop->randomopt_group[2] != 0)
				if ((g = itemdb_randomopt_group_exists(mobdrop->randomopt_group[2])) && g->total) {
					...
					if (&g->entries[r]) {
						//memcpy(&itm->option, &g->entries[r], sizeof(itm->option));
						g->entries[r].option[0].value = rnd_value(g->entries[r].option[0].value, (short)g->entries[r].option[0].param);
						memcpy(&itm->option[2], &g->entries[r].option[0], sizeof(s_item_randomoption));
					}
				}
			return;
		}
...
static struct item_drop* mob_setdropitem(struct s_mob_drop *mobdrop, int qty, unsigned short mob_id)
{
	...
	drop->item_data.identify = 1; //add identify mins   itemdb_isidentified(mobdrop->nameid);
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			// Nerguri 이벤트 명령어 Add Start
			if (event_dflag == 1) drop_rate = min((drop_rate + (drop_rate / 10) * event_rate),10000);
			// Nerguri End
...
static bool mob_readdb_drop(char* str[], int columns, int current) {
	...
	rate = atoi(str[2]);
	if (columns > 3 && (flag = atoi(str[3])) == 2) {
		drop = mob->mvpitem;
	...
	if (rate == 0) {
		...
		drop[i].randomopt_group[0] = 0;
		drop[i].randomopt_group[1] = 0;
		drop[i].randomopt_group[2] = 0;

		if (columns > 4) {
			int randomopt_group = -1;
			if (!script_get_constant(trim(str[4]), &randomopt_group)) {
			...
			drop[i].randomopt_group[0] = randomopt_group;
		}
		//add rndopt
		if (columns > 5) {
			int randomopt_group = -1;
			if (!script_get_constant(trim(str[5]), &randomopt_group)) {
				ShowError("mob_readdb_drop: Invalid 'randopt_groupid' '%s' for monster '%hu'.\n", str[5], mobid);
				return false;
			}
			if (randomopt_group == RDMOPTG_None)
			...
			drop[i].randomopt_group[1] = randomopt_group;
		}
		if (columns > 6) {
			int randomopt_group = -1;
			...
			drop[i].randomopt_group[2] = randomopt_group;
		}
	}
...
static void mob_load(void)
{
	...
	for(int i = 0; i < ARRAYLENGTH(dbsubpath); i++){	
		...
		sv_readdb(dbsubpath2, "mob_drop.txt", ',', 3, 7, -1, &mob_readdb_drop, silent);
