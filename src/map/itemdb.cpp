...
{
	const char* filename[] = {
		DBPATH"item_keepitem.txt",
		DBIMPORT"/item_keepitem.txt"
	};

	int fi;

	for (fi = 0; fi < ARRAYLENGTH(filename); ++fi) {
		uint32 lines = 0, count = 0, nameid = 0;
		char line[2048];
		...
		fp = fopen(path, "r");
		if (fp == NULL) {
			ShowWarning("item_read_keepitem: File not found \"%s\", skipping.\n", path);
			continue;
		}
		...
		while (fgets(line, sizeof(line), fp))
		{
			...
			if (p == NULL)
			{
				ShowError("itemdb_read_keepitem: \"%s\"파일의 %d번째 줄에서 잘못된 형식을 발견하여 무시합니다. (item id %d)\n", path, lines, nameid);
				continue;
			}
			...
			if (*p != '{')
			{
				ShowError("itemdb_read_keepitem: \"%s\"파일의 %d번째 줄의 스크립트 부분에서 잘못된 형식을 발견하여 무시합니다. (item id %d)\n", path, lines, nameid);
				continue;
			}
			str[6] = p;
		...
		ShowStatus("Done reading '" CL_WHITE "%lu" CL_RESET "' entries in '" CL_WHITE "item_keepitem" CL_RESET "'.\n", count);
	}
