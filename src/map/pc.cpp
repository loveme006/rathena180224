...
char pc_additem(struct map_session_data *sd,struct item *item,int amount,e_log_pick_type log_type) {
	...
	// 차암 시스템 Nerguri Start
	if (battle_config.keepitem_use && id->ki.exist)
		status_calc_pc(sd, SCO_NONE);
	//Nerguri End
...
{
	int nameid = 0; // [Nerguri] 차암 시스템 Add
	nullpo_retr(1, sd);
...
{
	struct item_data *data; // [Nerguri] 차암 시스템 Add
	nullpo_retr(1, sd);
	...
	// [Nerguri] 차암 시스템 Add Start
	data = itemdb_search(sd->inventory.u.items_inventory[n].nameid);

	if (battle_config.keepitem_use && data->ki.del)
		clif_displaymessage(sd->fd, "차암의 마법으로 드랍된 아이템은 사라졌습니다.");//Nerguri End
	else if (!map_addflooritem(&sd->inventory.u.items_inventory[n], amount, sd->bl.m, sd->bl.x, sd->bl.y, 0, 0, 0, 2, 0))
		return false;
...
void pc_gainexp(struct map_session_data *sd, struct block_list *src, unsigned int base_exp, unsigned int job_exp, uint8 exp_flag)
{
	...
	// @이벤트 명령어 [JaVb]
	if (event_eflag == 1) {
		base_exp = (unsigned int)(base_exp + (((float)base_exp / 10) * event_rate));
		job_exp = (unsigned int)(job_exp + (((float)job_exp / 10) * event_rate));
	}
