	...
	if (battle_config.keepitem_use)
	{
		for (i = 0; i < MAX_INVENTORY; i++)
		{
			if (sd->inventory_data[i])
			{
				...
				id = itemdb_search(sd->inventory.u.items_inventory[i].nameid);
				...
				if (id->ki.exist)
				{
					...
					if (id->ki.ref)
					{
						...
						{
							if (id->type == IT_ARMOR)
								refinedef += (refine_info[REFINE_TYPE_ARMOR].bonus[(sd->inventory.u.items_inventory[i].refine) - 1]) * sd->inventory.u.items_inventory[i].amount;
							else if (id->type == IT_WEAPON)
							{
								wlv = id->wlv;
								refinedef += (refine_info[wlv].bonus[(sd->inventory.u.items_inventory[i].refine) - 1]) * sd->inventory.u.items_inventory[i].amount;
							}
						}
					}
					if (id->ki.script)
					{
						if (id->ki.addeff)
							for (pp = 0; pp<sd->inventory.u.items_inventory[i].amount; pp++)
