...
BUILDIN_FUNC(failedrefitem) {
	...
	if (equip_index_check(pos))
		i = pc_checkequip(sd,equip_bitmask[pos]);
	// add mins
	short item_position = pc_search_inventory(sd, ITEMID_SMISS_BLESSING);
	short count6635 = 0;
	switch (sd->inventory.u.items_inventory[i].refine) {
		case 7:	count6635 = 1; 	break;
		case 8:	count6635 = 2; 	break;
		case 9:	count6635 = 4; 	break;
		case 10:	count6635 = 7; 	break;
		case 11:	count6635 = 11; 	break;
		default:	count6635 = 0; break;
	}

	if (i >= 0) {
		if (sd->inventory.u.items_inventory[item_position].amount < count6635 || count6635 == 0) {
			sd->inventory.u.items_inventory[i].refine = 0;
			pc_unequipitem(sd,i,3); //recalculate bonus
			pc_delitem(sd, i, 1, 0, 2, LOG_TYPE_SCRIPT);
		}
		else
			pc_delitem(sd, item_position, count6635, 0, 1, LOG_TYPE_CONSUME);
