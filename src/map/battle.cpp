...
static bool is_attack_critical(struct Damage wd, struct block_list *src, struct block_list *target, int skill_id, int skill_lv, bool first_call)
{
	...
	if(!(wd.type&DMG_MULTI_HIT) && sstatus->cri && (!skill_id ||
		...
		skill_id == TF_DOUBLE || skill_id == GS_CHAINACTION*/))
	{
...
int skill_mcri_hit(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	if (tmpd != NULL)
	{
		...
		case 1:
			...
			break;
		case 2:
			clif_skill_damage(tmpd->src, tmpd->bl, gettick(), 1, 1, id, 0, TK_STORMKICK, 1, DMG_MULTI_HIT);
	...
	if( sd && !skill_id ) {	// if no skill_id passed, check for double attack [helvetica]
		...
		{	//Success chance is not added, the higher one is used [Skotlex]
			...
			if( rnd()%100 < max_rate ) {
				...
				//		int64 damage;
				//		struct Damage dmg;
				//		dmg = battle_calc_attack(BF_WEAPON, src, target, skill_id, skill_lv, 0 & 0xFFF);
				...
				//		md->deletetimer = add_timer(gettick() + d_*num + 1, mob_timer_delete, md->bl.id, 0);
				//		status_set_viewdata(&md->bl, 32767);
				//		tmpd = &md->tmpd;
				...
				//		{
				//			damage = 1;
				//			status_change_start(src, target, SC_BLADESTOP_WAIT, 10000, 1, 0, 0, 0, u_, 2);
				//			status_change_start(src, target, SC_INVINCIBLE, 10000, 1, 0, 0, 0, u_, 2);
				//			add_timer(u_, skill_mcri_kill_delay, target->id, (intptr_t)src);
				//		}
				//		clif_skill_nodamage(src, src, skill_id, skill_lv, 1);
				//		for (i = 0; i < num; i++)
				//			add_timer(gettick() + d_*i + 1, skill_mcri_hit, _damage, (intptr_t)md);
				//		u_ = d_ = _damage = 0;
				//	}
				//}
			}
