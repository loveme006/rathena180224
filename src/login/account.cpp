...
static bool account_db_sql_create(AccountDB* self, struct mmo_account* acc, int max_acct);
 ...
 * @param max_acc: maximum number of accounts that can be created
 * @return true if successful, false if something has failed
 */
static bool account_db_sql_create(AccountDB* self, struct mmo_account* acc, int max_acct) {
	AccountDB_SQL* db = (AccountDB_SQL*)self;
	...
	{// ask the database
		...
		// [Nerguri] 계정 생성 개수 제한 Add Start
		if (max_acct > -1) {
			if (SQL_SUCCESS != Sql_Query(sql_handle, "SELECT count(*) FROM `%s` WHERE last_ip='%s'", db->account_db, acc->last_ip))
			{
				Sql_ShowDebug(sql_handle);
				return false;
			}
			...
			acc_count = (data != NULL) ? atoi(data) : 0;
			Sql_FreeResult(sql_handle);

			if (acc_count >= max_acct)
			{
				ShowError("IP [%s]에서 계정생성 최대가능 개수(%d개)를 초과하였습니다.\n", acc->last_ip, acc_count);
				return false;
			}
		}
		// Nerguri End
